...
PyramidCU::PyramidCU (SiftParam &sp)
  : SiftPyramid (sp)
{
  _allPyramid = NULL;
...
void
PyramidCU::InitPyramid (int w, int h, int ds)
{
  int wp, hp, toobig = 0;
...
void
PyramidCU::ResizePyramid (int w, int h)
{
  //
...
void
PyramidCU::FitPyramid (int w, int h)
{
  _pyramid_octave_first = 0;
  //
...
int
PyramidCU::CheckCudaDevice (int device)
{
  return ProgramCU::CheckCudaDevice (device);
}

void
PyramidCU::SetLevelFeatureNum (int idx, int fcount)
{
...
int
PyramidCU::ResizeFeatureStorage ()
{
  int totalkb = 0;
...
void
PyramidCU::GetFeatureDescriptors ()
{
  //descriptors...
...
void
PyramidCU::GenerateFeatureListTex ()
{
  vector<float> list;
  int idx = 0;
...
void
PyramidCU::ReshapeFeatureListCPU ()
{
  int i, szmax = 0, sz;
...
void
PyramidCU::GenerateFeatureDisplayVBO ()
{
...
void
PyramidCU::GenerateFeatureListCPU ()
{
...
void
PyramidCU::GenerateFeatureList (int i, int j, int reduction_count, vector<int> &hbuffer)
{
...
GLTexImage *
PyramidCU::GetLevelTexture (int octave, int level)
{
  return GetLevelTexture (octave, level, DATA_GAUSSIAN);
}

GLTexImage *
PyramidCU::ConvertTexCU2GL (CuTexImage *tex, int dataName)
{
...
GLTexImage *
PyramidCU::GetLevelTexture (int octave, int level, int dataName)
{
...
void
PyramidCU::ConvertInputToCU (GLTexInput *input)
{
...
void
PyramidCU::BuildPyramid (GLTexInput *input)
{
  USE_TIMING ();
      ...
      if (i == _octave_min)
        {
          ...
          if (i == 0)
            ...
            {
              if (i < 0)
                ProgramCU::SampleImageU (tex, _inputTex, -i);
              else
                ProgramCU::SampleImageD (tex, _inputTex, i);
...
int
PyramidCU::FitHistogramPyramid (CuTexImage *tex)
{
  CuTexImage *htex;
  int hist_level_num = _hpLevelNum - _pyramid_octave_first / 2;
...
void
PyramidCU::GetFeatureOrientations ()
{
  CuTexImage *ftex = _featureTex;
...
void
PyramidCU::GetSimplifiedOrientation ()
{
...
CuTexImage *
PyramidCU::GetBaseLevel (int octave, int dataName)
{
