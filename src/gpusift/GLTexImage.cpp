...
//for drawQuad
void
GLTexImage::SetImageSize (int width, int height)
{
...
void
GLTexImage::InitTexture (int width, int height, int clamp_to_edge)
{
  if (_texID && width == _texWidth && height == _texHeight)
    return;
  if (_texID == 0)
...
void
GLTexImage::InitTexture (int width, int height, int clamp_to_edge, GLuint format)
{
  if (_texID && width == _texWidth && height == _texHeight)
    return;
  if (_texID == 0)
...
void
GLTexImage::FillMargin (int marginx, int marginy)
{
  //
...
void
GLTexImage::ZeroHistoMargin ()
{
  ZeroHistoMargin (_imgWidth, _imgHeight);
}

void
GLTexImage::ZeroHistoMargin (int width, int height)
{
...
void
GLTexImage::DrawMargin (int right, int bottom)
{
  glBegin (GL_QUADS);
  if (right > _drawWidth)
    {
...
void
GLTexImage::DrawQuadDS (int scale)
{
  DrawScaledQuad (float(scale));
}

void
GLTexImage::DrawQuadUS (int scale)
{
  DrawScaledQuad (1.0f / scale);
}

void
GLTexImage::DrawScaledQuad (float texscale)
{
...
void
GLTexImage::DrawQuadReduction (int w, int h)
{
  float to = -0.5f;
  float tx = w * 2 + to;
  float ty = h * 2 + to;
...
void
GLTexPacked::SetImageSize (int width, int height)
{
  _imgWidth = width;
...
void
GLTexPacked::InitTexture (int width, int height, int clamp_to_edge)
{
  if (_texID && width == _imgWidth && height == _imgHeight)
    return;
  if (_texID == 0)
    glGenTextures (1, &_texID);
...
void
GLTexPacked::DrawQuadUS (int scale)
{
...
void
GLTexPacked::DrawQuadDS (int scale)
{
  int tw = _drawWidth;
  int th = _drawHeight;
...
void
GLTexPacked::FillMargin (int marginx, int marginy)
{
  //
...
void
GLTexPacked::DrawMargin (int right, int bottom, int mx, int my)
{
  int tw = (_imgWidth >> 1);
  int th = (_imgHeight >> 1);
...
void
GLTexImage::UnbindMultiTex (int n)
{
  for (int i = n - 1; i >= 0; i--)
    {
...
int
GLTexInput::DownSamplePixelDataF (unsigned int gl_format, int width, int height, int ds, const float *pin, float *pout, int skip)
{
  int step, linestep;
  int i, j;
  int ws = width / ds - skip;
...
int
GLTexInput::SetImageData (int width, int height, const void *data,
      ...
      if (simple_format)
        {
          if (GlobalUtil::_verbose)
            std::cout << "Automatic down-sampling is used\n";
          do
            {
              _down_sampled++;
              ws >>= 1;
  ...
  if (GlobalUtil::_verbose)
    {
      ...
      if (_down_sampled > 0)
        std::cout << "Down sample to \t" << ws << "x" << hs << "\n";
    }
...
int
GLTexInput::LoadImageFile (char *imagepath, int &w, int &h)
{
#ifndef SIFTGPU_NO_DEVIL
  ...
  if (done)
    SetImageData (width, height, data, GL_LUMINANCE, GL_UNSIGNED_BYTE);
  fclose (file);
  delete data;
  if (GlobalUtil::_verbose && done)
    std::cout << "Image loaded :\t" << imagepath << "\n";
  return 1;
#endif
}
...
int
GLTexImage::CopyToPBO (GLuint pbo, int width, int height, GLenum format)
{
  /////////
  if (format != GL_RGBA && format != GL_LUMINANCE)
    return 0;
...
void
GLTexImage::CopyFromPBO (GLuint pbo, int width, int height, GLenum format)
{
