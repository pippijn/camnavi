...
template<int FW>
__global__ void
FilterH (float *d_result, int width)
{
...
template<int FW>
__global__ void
FilterV (float *d_result, int width, int height)
{
  const int HALF_WIDTH = FW >> 1;
...
__global__ void
UpsampleKernel (float *d_result, int width)
{
...
////////////////////////////////////////////////////////////////////////////////////////
void
ProgramCU::SampleImageU (CuTexImage *dst, CuTexImage *src, int log_scale)
{
...
template<int LOG_SCALE>
__global__ void
DownsampleKernel (float *d_result, int src_width, int dst_width)
{
...
__global__ void
DownsampleKernel (float *d_result, int src_width, int dst_width, const int log_scale)
{
  const int dst_col = IMUL (blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
...
void
ProgramCU::SampleImageD (CuTexImage *dst, CuTexImage *src, int log_scale)
{
...
__global__ void
ChannelReduce_Kernel (float *d_result)
{
...
__global__ void
ChannelReduce_Convert_Kernel (float *d_result)
{
  int index = IMUL (blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
...
void
ProgramCU::ReduceToSingleChannel (CuTexImage *dst, CuTexImage *src, int convert_rgb)
{
...
__global__ void
ConvertByteToFloat_Kernel (float *d_result)
{
  int index = IMUL (blockIdx.x, FILTERH_TILE_WIDTH) + threadIdx.x;
...
void
ProgramCU::ConvertByteToFloat (CuTexImage *src, CuTexImage *dst)
{
  int width = src->GetImgWidth (), height = dst->GetImgHeight ();
  dim3 grid ((width * height + FILTERH_TILE_WIDTH - 1) / FILTERH_TILE_WIDTH);
  dim3 block (FILTERH_TILE_WIDTH);
...
void
ProgramCU::CreateFilterKernel (float sigma, float *kernel, int &width)
{
...
template<int FW>
void
ProgramCU::FilterImage (CuTexImage *dst, CuTexImage *src, CuTexImage *buf)
{
  int width = src->GetImgWidth (), height = src->GetImgHeight ();
...
// CUDA version: 28 ms
void
ProgramCU::FilterImage (CuTexImage *dst, CuTexImage *src, CuTexImage *buf, float sigma)
{
...
void __global__
ComputeDOG_Kernel (float *d_dog, float2 *d_got, int width, int height)
{
...
void __global__
ComputeDOG_Kernel (float *d_dog, int width, int height)
{
  int row = (blockIdx.y << DOG_BLOCK_LOG_DIMY) + threadIdx.y;
  int col = (blockIdx.x << DOG_BLOCK_LOG_DIMX) + threadIdx.x;
...
void
ProgramCU::ComputeDOG (CuTexImage *gus, CuTexImage *dog, CuTexImage *got)
{
...
void __global__
ComputeKEY_Kernel (float4 *d_key, int width, int colmax, int rowmax,
...
void
ProgramCU::ComputeKEY (CuTexImage *dog, CuTexImage *key, float Tdog, float Tedge)
{
...
void __global__
InitHist_Kernel (int4 *hist, int ws, int wd, int height)
{
...
void
ProgramCU::InitHistogram (CuTexImage *key, CuTexImage *hist)
{
...
void __global__
ReduceHist_Kernel (int4 *d_hist, int ws, int wd, int height)
{
  int row = IMUL (blockIdx.y, blockDim.y) + threadIdx.y;
  int col = IMUL (blockIdx.x, blockDim.x) + threadIdx.x;
...
void
ProgramCU::ReduceHistogram (CuTexImage *hist1, CuTexImage *hist2)
{
...
void __global__
ListGen_Kernel (int4 *d_list, int width)
{
...
//input list (x, y) (x, y) ....
void
ProgramCU::GenerateList (CuTexImage *list, CuTexImage *hist)
{
...
void __global__
ComputeOrientation_Kernel (float4 *d_list,
                           int list_len,
...
void
ProgramCU::ComputeOrientation (CuTexImage *list, CuTexImage *got, CuTexImage *key,
...
void __global__
ComputeDescriptor_Kernel (float4 *d_des, int num,
...
template<bool DYNAMIC_INDEXING>
void __global__
ComputeDescriptorRECT_Kernel (float4 *d_des, int num,
                              int width, int height, float window_factor)
{
  const float rpi = 4.0 / 3.14159265358979323846;
  int idx = IMUL (blockIdx.x, blockDim.x) + threadIdx.x;
...
void __global__
NormalizeDescriptor_Kernel (float4 *d_des, int num)
{
  float4 temp[32];
  int idx = IMUL (blockIdx.x, blockDim.x) + threadIdx.x;
...
void
ProgramCU::ComputeDescriptor (CuTexImage *list, CuTexImage *got, CuTexImage *dtex, int rect, int stream)
{
...
int
ProgramCU::CheckErrorCUDA (const char *location)
{
...
void __global__
ConvertDOG_Kernel (float *d_result, int width, int height)
{
...
///
void
ProgramCU::DisplayConvertDOG (CuTexImage *dog, CuTexImage *out)
{
  if (out->_cuData == NULL)
    return;
  int width = dog->GetImgWidth (), height = dog->GetImgHeight ();
...
void __global__
ConvertGRD_Kernel (float *d_result, int width, int height)
{
  int row = (blockIdx.y << BLOCK_LOG_DIM) + threadIdx.y;
  int col = (blockIdx.x << BLOCK_LOG_DIM) + threadIdx.x;
...
void
ProgramCU::DisplayConvertGRD (CuTexImage *got, CuTexImage *out)
{
  if (out->_cuData == NULL)
    return;
  int width = got->GetImgWidth (), height = got->GetImgHeight ();
...
void __global__
ConvertKEY_Kernel (float4 *d_result, int width, int height)
{
  int row = (blockIdx.y << BLOCK_LOG_DIM) + threadIdx.y;
  int col = (blockIdx.x << BLOCK_LOG_DIM) + threadIdx.x;
...
void
ProgramCU::DisplayConvertKEY (CuTexImage *key, CuTexImage *dog, CuTexImage *out)
{
  if (out->_cuData == NULL)
    return;
...
void __global__
DisplayKeyPoint_Kernel (float4 *d_result, int num)
{
  int idx = IMUL (blockIdx.x, blockDim.x) + threadIdx.x;
...
void
ProgramCU::DisplayKeyPoint (CuTexImage *ftex, CuTexImage *out)
{
...
void __global__
DisplayKeyBox_Kernel (float4 *d_result, int num)
{
  int idx = IMUL (blockIdx.x, blockDim.x) + threadIdx.x;
...
void
ProgramCU::DisplayKeyBox (CuTexImage *ftex, CuTexImage *out)
{
...
///////////////////////////////////////////////////////////////////
inline void
CuTexImage::BindTexture (textureReference &texRef)
{
  cudaBindTexture (NULL, &texRef, _cuData, &texRef.channelDesc, _numBytes);
}

inline void
CuTexImage::BindTexture2D (textureReference &texRef)
{
...
int
ProgramCU::CheckCudaDevice (int device)
{
...
void __global__
MultiplyDescriptor_Kernel (int *d_result, int num1, int num2, int3 *d_temp)
{
...
void
ProgramCU::MultiplyDescriptor (CuTexImage *des1, CuTexImage *des2, CuTexImage *texDot, CuTexImage *texCRT)
{
...
void __global__
MultiplyDescriptorG_Kernel (int *d_result, int num1, int num2, int3 *d_temp,
...
void
ProgramCU::MultiplyDescriptorG (CuTexImage *des1, CuTexImage *des2,
                                ...
                                float H[3][3], float hdistmax, float F[3][3], float fdistmax)
{
  ...
  texDot->InitTexture (num2, num1);
  if (texCRT)
    texCRT->InitTexture (num2, (num1 + MULT_BLOCK_DIMY - 1) / MULT_BLOCK_DIMY, 3);
...
void __global__
RowMatch_Kernel (int *d_dot, int *d_result, int num2, float distmax, float ratiomax)
{
...
void
ProgramCU::GetRowMatch (CuTexImage *texDot, CuTexImage *texMatch, float distmax, float ratiomax)
{
...
void __global__
ColMatch_Kernel (int3 *d_crt, int *d_result, int height, int num2, float distmax, float ratiomax)
{
...
void
ProgramCU::GetColMatch (CuTexImage *texCRT, CuTexImage *texMatch, float distmax, float ratiomax)
{
