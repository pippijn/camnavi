...
SiftMatchGL::SiftMatchGL (int max_sift, int use_glsl)
  : SiftMatchGPU ()
{
...
void
SiftMatchGL::SetMaxSift (int max_sift)
{
...
void
SiftMatchGL::AllocateSiftMatch ()
{
  //parameters, number of sift is limited by the texture size
  if (_max_sift > GlobalUtil::_texMaxDimGL)
    _max_sift = GlobalUtil::_texMaxDimGL;
  ///
...
void
SiftMatchGL::SetDescriptors (int index, int num, const unsigned char *descriptors, int id)
{
  if (_initialized == 0)
    return;
  if (index > 1)
...
void
SiftMatchGL::SetFeautreLocation (int index, const float *locations, int gap)
{
...
void
SiftMatchGL::SetDescriptors (int index, int num, const float *descriptors, int id)
{
  if (_initialized == 0)
    return;
  if (index > 1)
...
void
SiftMatchGL::LoadSiftMatchShadersGLSL ()
{
  ProgramGLSL *program;
  char buffer[10240];
...
void
SiftMatchGL::GetGuidedSiftMatch (MatchBuffer &match_buffer, float H[3][3], float F[3][3],
                                 float distmax, float ratiomax, float hdistmax, float fdistmax, int mbm)
{
  ...
  //set parameters glsl
  float dot_param[4] = {
    (float)_texDes[0].GetDrawHeight (), (float)_texDes[1].GetDrawHeight (), hdistmax, fdistmax
  };
...
void
SiftMatchGL::GetBestMatch (MatchBuffer &match_buffer, float distmax, float ratiomax, int mbm)
{
  glActiveTexture (GL_TEXTURE0);
  _texDot.BindTex ();
...
void
SiftMatchGL::GetSiftMatch (MatchBuffer &match_buffer, float distmax, float ratiomax, int mbm)
{
  int dw = _num_sift[1];
  int dh = _num_sift[0];
  ...
  //set parameters
  float heights[2] = {
    (float)_texDes[0].GetDrawHeight (), (float)_texDes[1].GetDrawHeight ()
  };
...
void *
SiftMatchGPU::operator new (size_t size)
{
  void *p = malloc (size);
...
void
SiftMatchGPU::SetLanguage (int language)
{
  if (__matcher)
    return;
  ////////////////////////
#ifdef CUDA_SIFTGPU_ENABLED
  if (language >= SIFTMATCH_CUDA)
    GlobalUtil::_DeviceIndex = language - SIFTMATCH_CUDA;
#endif
  __language = language > SIFTMATCH_CUDA ? SIFTMATCH_CUDA : language;
}

void
SiftMatchGPU::SetDeviceParam (int argc, char **argv)
{
  if (__matcher)
    return;
...
void
SiftMatchGPU::SetMaxSift (int max_sift)
{
  if (__matcher)
...
void
SiftMatchGPU::SetDescriptors (int index, int num, const unsigned char *descriptors, int id)
{
  __matcher->SetDescriptors (index, num, descriptors, id);
}

void
SiftMatchGPU::SetDescriptors (int index, int num, const float *descriptors, int id)
{
  __matcher->SetDescriptors (index, num, descriptors, id);
}

void
SiftMatchGPU::SetFeautreLocation (int index, const float *locations, int gap)
{
  __matcher->SetFeautreLocation (index, locations, gap);
}

void
SiftMatchGPU::GetGuidedSiftMatch (MatchBuffer &match_buffer, float H[3][3], float F[3][3],
                                  float distmax, float ratiomax, float hdistmax, float fdistmax, int mutual_best_match)
{
  if (H == NULL && F == NULL)
    __matcher->GetSiftMatch (match_buffer, distmax, ratiomax, mutual_best_match);
  else
    {
      float Z[3][3] = { { 1, 0, 0 }, { 0, 1, 0 }, { 0, 0, 1 } }, ti = (1.0e+20F);

      __matcher->GetGuidedSiftMatch (match_buffer, H ? H : Z, F ? F : Z,
                                     distmax, ratiomax, H ? hdistmax : ti, F ? fdistmax : ti, mutual_best_match);
    }
}

void
SiftMatchGPU::GetSiftMatch (MatchBuffer &match_buffer, float distmax, float ratiomax, int mutual_best_match)
{
  __matcher->GetSiftMatch (match_buffer, distmax, ratiomax, mutual_best_match);
}

SiftMatchGPU *
CreateNewSiftMatchGPU (int max_sift)
{
