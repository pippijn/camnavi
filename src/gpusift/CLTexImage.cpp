...
void
CLTexImage::SetContext (cl_context context, cl_command_queue queue)
{
  _context = context;
  _queue = queue;
}
...
void
CLTexImage::ReleaseTexture ()
{
  if (_fromGL)
    clEnqueueReleaseGLObjects (_queue, 1, &_clData, 0, NULL, NULL);
  if (_clData)
    clReleaseMemObject (_clData);
}

void
CLTexImage::SetImageSize (int width, int height)
{
  _imgWidth = width;
  _imgHeight = height;
}
...
void
CLTexImage::InitBufferTex (int width, int height, int nchannel)
{
  if (width == 0 || height == 0 || nchannel <= 0 || _fromGL)
    return;
  ...
  cl_int status;
  if (_clData)
    status = clReleaseMemObject (_clData);
...
void
CLTexImage::InitTexture (int width, int height, int nchannel)
{
  if (width == 0 || height == 0 || nchannel <= 0 || _fromGL)
    return;
  if (_clData && width == _texWidth && height == _texHeight && _numChannel == nchannel)
    return;
  if (_clData)
    clReleaseMemObject (_clData);
...
void
CLTexImage::InitPackedTex (int width, int height, int packed)
{
  if (packed)
...
void
CLTexImage::SetPackedSize (int width, int height, int packed)
{
  if (packed)
    SetImageSize ((width + 1) >> 1, (height + 1) >> 1);
  else
    SetImageSize (width, height);
}

void
CLTexImage::InitTextureGL (GLuint tex, int width, int height, int nchannel)
{
  if (tex == 0)
    return;
  if (_clData)
...
void
CLTexImage::CopyFromHost (const void *buf)
{
  if (_clData == NULL)
    return;
  cl_int status;
...
int
CLTexImage::GetImageDataSize ()
{
  return _imgWidth * _imgHeight * _numChannel * sizeof (float);
}

int
CLTexImage::CopyToPBO (GLuint pbo)
{
...
void
CLTexImage::CopyToHost (void *buf)
{
  if (_clData == NULL)
    return;
  cl_int status;
