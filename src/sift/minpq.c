 ...
 */
int
minpq_insert (struct min_pq *min_pq, void *data, int key)
{
  int n = min_pq->n;
 ...
 */
void *
minpq_get_min (struct min_pq *min_pq)
{
  if (min_pq->n < 1)
    {
 ...
 */
void *
minpq_extract_min (struct min_pq *min_pq)
{
  void *data;
 ...
 */
void
minpq_release (struct min_pq **min_pq)
{
  if (!min_pq)
    {
 ...
 */
static void
decrease_pq_node_key (struct pq_node *pq_array, int i, int key)
{
  struct pq_node tmp;
 ...
 */
static void
restore_minpq_order (struct pq_node *pq_array, int i, int n)
{
  struct pq_node tmp;
  int l, r, min = i;
