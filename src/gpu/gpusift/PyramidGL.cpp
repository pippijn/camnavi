...
//align must be 2^i
void
PyramidGL::GetAlignedStorageSize (int num, int align, int &fw, int &fh)
{
  if (num <= 0)
    fw = fh = 0;
...
void
PyramidGL::GetTextureStorageSize (int num, int &fw, int &fh)
{
  if (num <= 0)
    fw = fh = 0;
...
void
PyramidNaive::DestroyPyramidData ()
{
  if (_texPyramid)
    {
      delete[] _texPyramid;
...
void
PyramidNaive::FitHistogramPyramid ()
{
  GLTexImage *tex, *htex;
...
void
PyramidNaive::FitPyramid (int w, int h)
{
...
void
PyramidNaive::InitPyramid (int w, int h, int ds)
{
  int wp, hp, toobig = 0;
...
void
PyramidNaive::ResizePyramid (int w, int h)
{
  //
  ...
  if (_octave_num_new < 1)
    _octave_num_new = GetRequiredOctaveNum (input_sz);
  ...
  if (_texPyramid == NULL)
    _texPyramid = new GLTexImage[noct * nlev];
  if (_auxPyramid == NULL)
    _auxPyramid = new GLTexImage[noct * nlev];
...
int
PyramidGL::ResizeFeatureStorage ()
{
  int totalkb = 0;
  ...
  //
  if (_histo_buffer == NULL)
    _histo_buffer = new float[1 << (2 + 2 * GlobalUtil::_ListGenSkipGPU)];
...
void
PyramidNaive::BuildPyramid (GLTexInput *input)
{
  USE_TIMING ();
  GLTexPacked *tex;
...
GLTexImage *
PyramidNaive::GetLevelTexture (int octave, int level, int dataName)
{
...
GLTexImage *
PyramidNaive::GetLevelTexture (int octave, int level)
{
...
GLTexImage *
PyramidNaive::GetBaseLevel (int octave, int dataName)
{
  if (octave < _octave_min || octave > _octave_min + _octave_num)
    return NULL;
  switch (dataName)
...
void
PyramidNaive::DetectKeypointsEX ()
{
  int i, j;
  ...
  int gradient_only_levels[2] = {
    param._level_min + 1, param._level_max
  };
...
void
PyramidNaive::GenerateFeatureList (int i, int j)
{
  int hist_level_num = _hpLevelNum - _pyramid_octave_first;
            ...
            for (int c = 0; c < 4; c++, pos++)
              {
                ...
                if (ss == 0)
                  continue;
                float ft[4] = {
                  2 * x + (c % 2 ? 1.5f :  0.5f), 2 * y + (c >= 2 ? 1.5f : 0.5f), 0, 1
                };
                for (int t = 0; t < ss; t++)
                  {
                    ft[2] = (float)t;
...
void
PyramidNaive::GenerateFeatureList ()
{
...
void
PyramidGL::GenerateFeatureDisplayVBO ()
{
...
void
PyramidNaive::GetFeatureOrientations ()
{
  GLTexImage *gtex;
...
void
PyramidNaive::GenerateFeatureListCPU ()
{
  FrameBufferObject fbo;
          ...
          for (int m = 0; m < tex->GetTexWidth (); m++, p++)
            {
              if (*p == 0)
                continue;
              if (m == 0 || k == 0 || k >= tex->GetImgHeight () - 1 || m >= tex->GetImgWidth () - 1)
                continue;
...
void
PyramidGL::ReshapeFeatureListCPU ()
{
  ...
  if (_down_sample_factor > 0)
    os *= float(1 << _down_sample_factor);
  ...
  for (i = 0; i < n; i++)
    {
      ...
      int sz = nfh * nfw;
      if (sz > fcount)
        memset (des, 0, sizeof (float) * (sz - fcount) * 4);
...
inline void
PyramidGL::SetLevelFeatureNum (int idx, int fcount)
{
  int fw, fh;
  GLTexImage *ftex = _featureTex + idx;
...
void
PyramidNaive::GetSimplifiedOrientation ()
{
  //
  int idx = 0;
...
static inline void
multiply_and_truncate_128d (float *p, float m)
{
  float z = 0.2f;
...
inline void
PyramidGL::NormalizeDescriptor (int num, float *pd)
{
#ifdef USE_SSE_FOR_SIFTGPU
...
inline void
PyramidGL::InterlaceDescriptorF2 (int w, int h, float *buf, float *pd, int step)
{
  /*
...
void
PyramidGL::GetFeatureDescriptors ()
{
  //descriptors...
  float sigma;
  int idx, i, j, k, w, h;
      ...
      for (j = 0; j < param._dog_level_num; j++, ftex++, idx++, gtex++, otex++)
        {
          ...
          if (w > _descriptorTex[0].GetTexWidth () || h > _descriptorTex[0].GetTexHeight ())
            for (k = 0; k < ndf; k++)
              _descriptorTex[k].InitTexture (w, h);
          for (k = 0; k < ndf; k++)
...
void
PyramidGL::GenerateFeatureListTex ()
{
      ...
      for (int j = 0; j < param._dog_level_num; j++, idx++)
        {
          ...
          if (fcount == _featureNum)
            _keypoint_index.resize (0);
        }
      if (GlobalUtil::_KeyPointListForceLevel0)
        break;
    }
  GLTexImage::UnbindTex ();
  if (GlobalUtil::_verbose)
...
void
PyramidPacked::BuildPyramid (GLTexInput *input)
{
  //
  USE_TIMING ();
  GLTexImage *tex, *tmp;
...
void
PyramidPacked::DetectKeypointsEX ()
{
  //first pass, compute dog, gradient, orientation
  GLenum buffers[4] = {
    GL_COLOR_ATTACHMENT0_EXT, GL_COLOR_ATTACHMENT1_EXT,
...
void
PyramidPacked::GenerateFeatureList (int i, int j)
{
  float fcount = 0.0f;
  int hist_skip_gpu = GlobalUtil::_ListGenSkipGPU;
  int idx = i * param._dog_level_num + j;
...
void
PyramidPacked::GenerateFeatureList ()
{
...
void
PyramidPacked::GenerateFeatureListCPU ()
{
  FrameBufferObject fbo;
...
void
PyramidPacked::GetFeatureOrientations ()
{
...
void
PyramidPacked::GetSimplifiedOrientation ()
{
  //
  int idx = 0;
  //	int n = _octave_num  * param._dog_level_num;
...
void
PyramidPacked::InitPyramid (int w, int h, int ds)
{
  int wp, hp, toobig = 0;
...
void
PyramidPacked::FitPyramid (int w, int h)
{
  //(w, h) <= (_pyramid_width, _pyramid_height);
  ...
  for (int i = 0; i < _octave_num; i++)
    {
      ...
      for (int j = param._level_min; j <= param._level_max; j++, tex++, dog++, grd++, rot++, key++)
        {
          tex->SetImageSize (w, h);
          if (j == param._level_min)
            continue;
          dog->SetImageSize (w, h);
          grd->SetImageSize (w, h);
          rot->SetImageSize (w, h);
          if (j == param._level_min + 1 || j == param._level_max)
            continue;
...
void
PyramidPacked::ResizePyramid (int w, int h)
{
  //
  unsigned int totalkb = 0;
  ...
  //	//initialize the pyramid
  if (_allPyramid == NULL)
    _allPyramid = new GLTexPacked[noct * nlev * DATA_NUM];
...
void
PyramidPacked::DestroyPyramidData ()
{
  if (_allPyramid)
    {
      delete[] _allPyramid;
...
GLTexImage *
PyramidPacked::GetLevelTexture (int octave, int level, int dataName)
{
...
GLTexImage *
PyramidPacked::GetLevelTexture (int octave, int level)
{
  return _allPyramid + (_pyramid_octave_first + octave - _octave_min) * param._level_num
         + (level - param._level_min);
}
...
GLTexImage *
PyramidPacked::GetBaseLevel (int octave, int dataName)
{
  if (octave < _octave_min || octave > _octave_min + _octave_num)
    return NULL;
...
void
PyramidPacked::FitHistogramPyramid ()
{
  GLTexImage *tex, *htex;
  int hist_level_num = _hpLevelNum - _pyramid_octave_first;
