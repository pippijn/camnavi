...
ProgramCL::ProgramCL (const char *name, const char *code, cl_context context, cl_device_id device)
  : _valid (1)
{
  const char *src[1] = {
    code
  ...
  if (status != CL_SUCCESS)
    {
      PrintBuildLog (device, 1);
      _valid = 0;
    }
  else if (GlobalUtil::_debug)
    PrintBuildLog (device, 0);
...
void
ProgramCL::PrintBuildLog (cl_device_id device, int all)
{
  char buffer[10240] = "\0";
...
bool
ProgramBagCL::InitializeContext ()
{
  ...
  _device = (status == CL_SUCCESS ? devices[0] : 0);
  delete[] devices;
  if (status != CL_SUCCESS)
    return false;
  ...
  if (GlobalUtil::_verbose)
    {
      ...
      if (is_gcache == CL_NONE)
        std::cout << "No cache for global memory\n";
...
void
ProgramBagCL::InitProgramBag (SiftParam &param)
{
...
void
ProgramBagCL::UnloadProgram ()
{
}

void
ProgramBagCL::FinishCL ()
{
  clFinish (_queue);
}

void
ProgramBagCL::LoadFixedShaders ()
{
  ...
  s_grad_pass2 = new ProgramCL ("grad_pass2",
                                "#define BLOCK_DIMX 32u\n"
                                "#define BLOCK_DIMY 14u\n"
...
void
ProgramBagCL::LoadDynamicShaders (SiftParam &param)
{
  LoadKeypointShader ();
...
void
ProgramBagCL::SelectInitialSmoothingFilter (int octave_min, SiftParam &param)
{
...
void
ProgramBagCL::CreateGaussianFilters (SiftParam &param)
{
...
FilterCL *
ProgramBagCL::CreateGaussianFilter (float sigma)
{
...
FilterCL *
ProgramBagCL::CreateFilter (float kernel[], int width)
{
...
ProgramCL *
ProgramBagCL::CreateFilterH (float kernel[], int width)
{
...
ProgramCL *
ProgramBagCL::CreateFilterV (float kernel[], int width)
{
  int halfwidth = width >> 1;
  float *pf = kernel + halfwidth;
  int nhpixel = (halfwidth + 1) >> 1;   //how many neighbour pixels need to be looked up
...
void
ProgramBagCL::FilterImage (FilterCL *filter, CLTexImage *dst, CLTexImage *src, CLTexImage *tmp)
{
  ...
  size_t gsz[2] = {
    (w + dim0 - 1) / dim0 * dim0, (h + dim1 - 1) / dim1 * dim1
  }, lsz[2] = {
    dim0, dim1
  };
  ...
  size_t gsz2[2] = {
    (w + dim1 - 1) / dim1 * dim1, (h + dim0 - 1) / dim0 * dim0
  }, lsz2[2] = {
    dim1, dim0
  };
...
void
ProgramBagCL::SampleImageU (CLTexImage *dst, CLTexImage *src, int log_scale)
{
...
void
ProgramBagCL::SampleImageD (CLTexImage *dst, CLTexImage *src, int log_scale)
{
  cl_kernel kernel;
  cl_int w = dst->GetImgWidth (), h = dst->GetImgHeight ();
...
void
ProgramBagCL::FilterInitialImage (CLTexImage *tex, CLTexImage *buf)
{
  if (f_gaussian_skip0)
    FilterImage (f_gaussian_skip0, tex, tex, buf);
}

void
ProgramBagCL::FilterSampledImage (CLTexImage *tex, CLTexImage *buf)
{
  if (f_gaussian_skip1)
    FilterImage (f_gaussian_skip1, tex, tex, buf);
}

void
ProgramBagCL::ComputeDOG (CLTexImage *tex, CLTexImage *texp, CLTexImage *dog, CLTexImage *grad, CLTexImage *rot)
{
  ...
  if (!both_grad_dog)
    {
      kernel = s_dog_pass->_kernel;
      dim0 = 16;
      dim1 = 12;
    }
  else if (use_gm2)
    {
      kernel = s_grad_pass2->_kernel;
      dim0 = 32;
      dim1 = 14;
      margin = 2;
    }
    ...
    {
      kernel = s_grad_pass->_kernel;
      dim0 = 16;
      dim1 = 20;
    }
  size_t gsz[2] = {
    (w + dim0 - 1 - margin) / (dim0 - margin) * dim0,
...
void
ProgramBagCL::ComputeKEY (CLTexImage *dog, CLTexImage *key, float Tdog, float Tedge)
{
...
void
ProgramBagCL::UnpackImage (CLTexImage *src, CLTexImage *dst)
{
...
void
ProgramBagCL::UnpackImageDOG (CLTexImage *src, CLTexImage *dst)
{
  if (s_unpack_dog == NULL)
    return;
...
void
ProgramBagCL::UnpackImageGRD (CLTexImage *src, CLTexImage *dst)
{
  if (s_unpack_grd == NULL)
    return;
...
void
ProgramBagCL::UnpackImageKEY (CLTexImage *src, CLTexImage *dog, CLTexImage *dst)
{
  if (s_unpack_key == NULL)
    return;
...
void
ProgramBagCL::LoadDescriptorShader ()
{
...
void
ProgramBagCL::LoadDescriptorShaderF2 ()
{
}

void
ProgramBagCL::LoadOrientationShader (void)
{
}

void
ProgramBagCL::LoadGenListShader (int ndoglev, int nlev)
{
}

void
ProgramBagCL::LoadKeypointShader ()
{
  int i;
  char buffer[20240];
...
void
ProgramBagCL::LoadDisplayShaders ()
{
...
void
ProgramBagCL::SetMarginCopyParam (int xmax, int ymax)
{
}

void
ProgramBagCL::SetGradPassParam (int texP)
{
}

void
ProgramBagCL::SetGenListEndParam (int ktex)
{
}

void
ProgramBagCL::SetDogTexParam (int texU, int texD)
{
}

void
ProgramBagCL::SetGenListInitParam (int w, int h)
{
  float bbox[4] = {
...
void
ProgramBagCL::SetGenListStartParam (float width, int tex0)
{
}

void
ProgramBagCL::SetGenListStepParam (int tex, int tex0)
{
}

void
ProgramBagCL::SetGenVBOParam (float width, float fwidth, float size)
{
}

void
ProgramBagCL::SetSimpleOrientationInput (int oTex, float sigma, float sigma_step)
{
}

void
ProgramBagCL::SetFeatureOrientationParam (int gtex, int width, int height, float sigma, int otex, float step)
{
}

void
ProgramBagCL::SetFeatureDescirptorParam (int gtex, int otex, float dwidth, float fwidth, float width, float height, float sigma)
{
}

const char *
ProgramBagCL::GetErrorString (cl_int error)
{
...
bool
ProgramBagCL::CheckErrorCL (cl_int error, const char *location)
{
  if (error == CL_SUCCESS)
    return true;
...
void
ProgramBagCLN::LoadFixedShaders ()
{
  s_sampling = new ProgramCL ("sampling",
                              "const sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;\n"
                              "__kernel void sampling(__read_only  image2d_t input, __write_only image2d_t output, "
...
void
ProgramBagCLN::LoadDisplayShaders ()
{
  s_unpack = new ProgramCL ("main",
                            "const sampler_t sampler = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST;\n"
                            "__kernel void main(__read_only  image2d_t input, __write_only image2d_t output,\n"
...
ProgramCL *
ProgramBagCLN::CreateFilterH (float kernel[], int width)
{
  ////////////////////////////
  char buffer[10240];
  ostrstream out (buffer, 10240);
...
ProgramCL *
ProgramBagCLN::CreateFilterV (float kernel[], int width)
{
  ////////////////////////////
  char buffer[10240];
  ostrstream out (buffer, 10240);
...
FilterCL *
ProgramBagCLN::CreateFilter (float kernel[], int width)
{
  FilterCL *filter = new FilterCL;
...
void
ProgramBagCLN::FilterImage (FilterCL *filter, CLTexImage *dst, CLTexImage *src, CLTexImage *tmp)
{
  cl_kernel kernelh = filter->s_shader_h->_kernel;
  cl_kernel kernelv = filter->s_shader_v->_kernel;
  //////////////////////////////////////////////////////////////////
  ...
  size_t gsz1[2] = {
    (w + dim00 - 1) / dim00 * dim00, (h + dim01 - 1) / dim01 * dim01
  }, lsz1[2] = {
    dim00, dim01
  };
  ...
  size_t gsz2[2] = {
    (w + dim10 - 1) / dim10 * dim10, (h + dim11 - 1) / dim11 * dim11
  }, lsz2[2] = {
    dim10, dim11
  };
  status = clEnqueueNDRangeKernel (_queue, kernelv, 2, NULL, gsz2, lsz2, 0, NULL, NULL);
...
void
ProgramBagCLN::SampleImageD (CLTexImage *dst, CLTexImage *src, int log_scale)
{
  cl_kernel kernel;
  cl_int w = dst->GetImgWidth (), h = dst->GetImgHeight ();
