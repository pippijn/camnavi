...
int
ProgramGLSL::ShaderObject::ReadShaderFile (const char *sourcefile, char * &code)
{
  code = NULL;
  FILE *file;
  int len = 0;
...
void
ProgramGLSL::ShaderObject::CheckCompileLog ()
{
  GLint status;
...
int
ProgramGLSL::ShaderObject::IsValidFragmentShader ()
{
  return _type == GL_FRAGMENT_SHADER && _shaderID && _compiled;
}

int
ProgramGLSL::ShaderObject::IsValidVertexShader ()
{
  return _type == GL_VERTEX_SHADER && _shaderID && _compiled;
}

void
ProgramGLSL::ShaderObject::PrintCompileLog (ostream &os)
{
  GLint len = 0;
...
void
ProgramGLSL::AttachShaderObject (ShaderObject &shader)
{
...
void
ProgramGLSL::DetachShaderObject (ShaderObject &shader)
{
  if (_programID && shader.IsValidShaderObject ())
...
void
ProgramGLSL::PrintLinkLog (std::ostream &os)
{
  GLint len = 0;
...
void
FilterGLSL::MakeFilterProgram (float kernel[], int width)
{
...
ProgramGPU *
FilterGLSL::CreateFilterH (float kernel[], int width)
{
  char buffer[10240];
...
ProgramGPU *
FilterGLSL::CreateFilterV (float kernel[], int height)
{
  char buffer[10240];
  ostrstream out (buffer, 10240);
...
ProgramGPU *
FilterGLSL::CreateFilterHPK (float kernel[], int width)
{
...
ProgramGPU *
FilterGLSL::CreateFilterVPK (float kernel[], int height)
{
  //both h and v are packed...
  int i, j, yw, ywn;
...
ShaderBag::~ShaderBag ()
{
  if (s_debug)
    delete s_debug;
  if (s_orientation)
    delete s_orientation;
  if (s_display_gaussian)
    delete s_display_gaussian;
  if (s_display_dog)
    delete s_display_dog;
  if (s_display_grad)
    delete s_display_grad;
  if (s_display_keys)
    delete s_display_keys;
  if (s_sampling)
    delete s_sampling;
  if (s_grad_pass)
    delete s_grad_pass;
  ...
  if (s_keypoint)
    delete s_keypoint;
  if (s_genlist_init_tight)
    delete s_genlist_init_tight;
  if (s_genlist_init_ex)
    delete s_genlist_init_ex;
  if (s_genlist_histo)
    delete s_genlist_histo;
  if (s_genlist_start)
    delete s_genlist_start;
  if (s_genlist_step)
    delete s_genlist_step;
  if (s_genlist_end)
    delete s_genlist_end;
  if (s_vertex_list)
    delete s_vertex_list;
  if (s_descriptor_fp)
    delete s_descriptor_fp;
  if (s_margin_copy)
    delete s_margin_copy;
...
void
ShaderBag::SelectInitialSmoothingFilter (int octave_min, SiftParam &param)
{
...
void
ShaderBag::CreateGaussianFilters (SiftParam &param)
{
...
void
ShaderBag::LoadDynamicShaders (SiftParam &param)
{
...
void
ShaderBagGLSL::LoadFixedShaders ()
{
...
void
ShaderBagGLSL::LoadDisplayShaders ()
{
...
void
ShaderBagGLSL::LoadKeypointShader (float threshold, float edge_threshold)
{
  char buffer[10240];
...
void
ShaderBagGLSL::SetDogTexParam (int texU, int texD)
{
...
void
ShaderBagGLSL::SetGenListStepParam (int tex, int tex0)
{
  glUniform1i (_param_genlist_step_tex0, 1);
}

void
ShaderBagGLSL::SetGenVBOParam (float width, float fwidth, float size)
{
  float sizes[4] = {
    size *3.0f, fwidth, width, 1.0f / width
  };
...
void
ShaderBagGLSL::UnloadProgram ()
{
  glUseProgram (0);
}

void
ShaderBagGLSL::LoadGenListShader (int ndoglev, int nlev)
{
  ProgramGLSL *program;
...
void
ShaderBagGLSL::SetMarginCopyParam (int xmax, int ymax)
{
  float truncate[2] = {
    xmax - 0.5f, ymax - 0.5f
  };
...
void
ShaderBagGLSL::SetGenListInitParam (int w, int h)
{
  float bbox[2] = {
    w - 1.0f, h - 1.0f
  };
...
void
ShaderBagGLSL::SetGenListStartParam (float width, int tex0)
{
  glUniform1f (_param_ftex_width, width);
}

ProgramGLSL *
ShaderBagGLSL::LoadGenListStepShader (int start, int step)
{
  int i;
  char buffer[10240];
  // char chanels[5] = "rgba";
  ...
  for (i = 0; i < step; i++)
    out << "uniform sampler2DRect tex" << i << ";\n";
  if (start)
    {
...
void
ShaderBagGLSL::LoadOrientationShader ()
{
  char buffer[10240];
  ostrstream out (buffer, 10240);
...
void
ShaderBagGLSL::WriteOrientationCodeToStream (std::ostream &out)
{
...
void
ShaderBagGLSL::SetSimpleOrientationInput (int oTex, float sigma, float sigma_step)
{
...
void
ShaderBagGLSL::SetFeatureOrientationParam (int gtex, int width, int height, float sigma, int stex, float step)
{
  ///
  glUniform1i (_param_orientation_gtex, 1);
...
void
ShaderBagGLSL::LoadDescriptorShaderF2 ()
{
...
void
ShaderBagGLSL::LoadDescriptorShader ()
{
...
void
ShaderBagGLSL::SetFeatureDescirptorParam (int gtex, int otex, float dwidth, float fwidth, float width, float height, float sigma)
{
  ///
  glUniform1i (_param_descriptor_gtex, 1);

  float dsize[4] = {
    dwidth, 1.0f / dwidth, fwidth, 1.0f / fwidth
  };
...
void
ShaderBagPKSL::LoadFixedShaders ()
{
  ProgramGLSL *program;
...
void
ShaderBagPKSL::LoadOrientationShader (void)
{
  char buffer[10240];
  ostrstream out (buffer, 10240);
...
void
ShaderBagPKSL::SetGenListStartParam (float width, int tex0)
{
  glUniform1f (_param_ftex_width, width);
}

void
ShaderBagPKSL::LoadGenListShader (int ndoglev, int nlev)
{
  ProgramGLSL *program;
...
void
ShaderBagPKSL::UnloadProgram (void)
{
  glUseProgram (0);
}

void
ShaderBagPKSL::LoadKeypointShader (float dog_threshold, float edge_threshold)
{
  //
  char buffer[10240];
...
void
ShaderBagPKSL::SetDogTexParam (int texU, int texD)
{
  glUniform1i (_param_dog_texu, 1);
  glUniform1i (_param_dog_texd, 2);
}
...
void
ShaderBagPKSL::SetGenListStepParam (int tex, int tex0)
{
  glUniform1i (_param_genlist_step_tex0, 1);
}

void
ShaderBagPKSL::SetGenVBOParam (float width, float fwidth, float size)
{
  float sizes[4] = {
    size *3.0f, fwidth, width, 1.0f / width
  };
...
void
ShaderBagPKSL::SetGradPassParam (int texP)
{
  glUniform1i (_param_grad_pass_texp, 1);
}

void
ShaderBagPKSL::LoadDescriptorShader ()
{
  GlobalUtil::_DescriptorPPT = 16;
  LoadDescriptorShaderF2 ();
...
ProgramGLSL *
ShaderBagPKSL::LoadDescriptorProgramRECT ()
{
  //one shader outpout 128/8 = 16 , each fragout encodes 4
  //const double twopi = 2.0*3.14159265358979323846;
  //const double rpi  = 8.0/twopi;
...
ProgramGLSL *
ShaderBagPKSL::LoadDescriptorProgramPKSL ()
{
  //one shader outpout 128/8 = 16 , each fragout encodes 4
  //const double twopi = 2.0*3.14159265358979323846;
  //const double rpi  = 8.0/twopi;
...
void
ShaderBagPKSL::LoadDescriptorShaderF2 ()
{
...
void
ShaderBagPKSL::SetSimpleOrientationInput (int oTex, float sigma, float sigma_step)
{
  glUniform1i (_param_orientation_gtex, 1);
...
void
ShaderBagPKSL::SetFeatureOrientationParam (int gtex, int width, int height, float sigma, int otex, float step)
{
  ///
  glUniform1i (_param_orientation_gtex, 1);
...
void
ShaderBagPKSL::SetFeatureDescirptorParam (int gtex, int otex, float dwidth, float fwidth, float width, float height, float sigma)
{
...
void
ShaderBagPKSL::SetGenListEndParam (int ktex)
{
  glUniform1i (_param_genlist_end_ktex, 1);
}

void
ShaderBagPKSL::SetGenListInitParam (int w, int h)
{
  float bbox[4] = {
    (w - 1.0f) * 0.5f + 0.25f, (w - 1.0f) * 0.5f - 0.25f, (h - 1.0f) * 0.5f + 0.25f, (h - 1.0f) * 0.5f - 0.25f
  };
...
void
ShaderBagPKSL::SetMarginCopyParam (int xmax, int ymax)
{
  float truncate[4];
