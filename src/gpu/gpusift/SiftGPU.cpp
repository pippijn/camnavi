...
void *
SiftGPU::operator new (size_t size)
{
  void *p = malloc (size);

  if (p == 0)
    {
      const std::bad_alloc ba; throw ba;
    }
  return p;
}

void
SiftGPUEX::RandomizeColor ()
{
  float hsv[3] = {
    0, 0.8f, 1.0f
  };
...
SiftGPU::InitSiftGPU ()
{
  if (_initialized || GlobalUtil::_GoodOpenGL == 0)
    return;
...
int
SiftGPU::RunSIFT (int width, int height, const void *data, unsigned int gl_format, unsigned int gl_type)
{
  if (GlobalUtil::_GoodOpenGL == 0)
    return 0;
  if (!_initialized)
...
int
SiftGPU::RunSIFT (const char *imgpath)
{
...
int
SiftGPU::RunSIFT (int num, const SiftKeypoint *keys, int keys_have_orientation)
{
  if (num <= 0)
    return 0;
...
{
  //check image data
  if (_imgpath[0] == 0 && _image_loaded == 0)
    return 0;
  ...
  if (GlobalUtil::_ExitAfterSIFT && GlobalUtil::_UseSiftGPUEX)
    exit (0);
...
void
SiftGPU::SetKeypointList (int num, const SiftKeypoint *keys, int keys_have_orientation)
{
  _pyramid->SetKeypointList (num, (const float *)keys, 0, keys_have_orientation);
}

void
SiftGPUEX::DisplayInput ()
{
  if (_texImage == NULL)
    return;
...
void
SiftGPU::SetVerbose (int verbose)
{
...
float
SiftParam::GetInitialSmoothSigma (int octave_min)
{
...
void
SiftParam::ParseSiftParam ()
{
  if (_dog_level_num == 0)
    _dog_level_num = 3;
  if (_level_max == 0)
    _level_max = _dog_level_num + 1;
  if (_sigma0 == 0.0f)
    _sigma0 = 1.6f * powf (2.0f, 1.0f / _dog_level_num);
  if (_sigman == 0.0f)
    _sigman = 0.5f;
  ...
  if (_level_ds > _level_max)
    _level_ds = _level_max;
  ...
  if (_dog_threshold == 0)
    _dog_threshold = 0.02f / _dog_level_num;
  if (_edge_threshold == 0)
    _edge_threshold = 10.0f;
}

void
SiftGPUEX::DisplayOctave (void (*UseDisplayShader)(), int i)
{
  if (_pyramid == NULL)
    return;
  ...
  if (_pyramid->_octave_min > 0)
    scale *= (1 << _pyramid->_octave_min);
  else if (_pyramid->_octave_min < 0)
    scale /= (1 << (-_pyramid->_octave_min));


  i = i % _pyramid->_octave_num;       //
  if (i < 0)
    i += _pyramid->_octave_num;
...
void
SiftGPUEX::DisplayPyramid (void (*UseDisplayShader)(), int dataName, int nskip1, int nskip2)
{
  if (_pyramid == NULL)
    return;
  int grid_sz = (_level_num - nskip1 - nskip2);
  if (grid_sz > 4)
    grid_sz = (int)ceil (grid_sz * 0.5);
  double scale = 1.0 / grid_sz;
      ...
      for (int j = _level_min + nskip1; j <= _level_max - nskip2; j++)
        {
          ...
          if (tex->GetImgWidth () == 0 || tex->GetImgHeight () == 0)
            continue;
...
void
SiftGPUEX::DisplayLevel (void (*UseDisplayShader)(), int i)
{
  if (_pyramid == NULL)
    return;
  ...
  if (i < 0)
    i += (_level_num * _pyramid->_octave_num);
...
void
SiftGPUEX::SetView (int view, int sub_view, char *title)
{
...
SiftGPU::PrintUsage ()
{
  ...
  << "-fs <int>         : Block Size for feature storage <default : 4>\n"
...
void
SiftGPU::ParseParam (int argc, char **argv)
{
  ...
  for (int i = 0; i < argc; i++)
    {
      ...
      switch (opti)
        ...
        case MAKEINT4 (s, i, g, n):
          if (!_initialized || GlobalUtil::_UseCUDA)
...
void
SiftGPU::SetImageList (int nimage, const char **filelist)
{
  _list->resize (0);
...
void
SiftGPU::LoadImageList (char *imlist)
{
...
void
SiftGPUEX::ToggleDisplayDebug ()
{
  _view_debug = !_view_debug;
}

void
SiftGPUEX::DisplayDebug ()
{
  glPointSize (1.0f);
  glColor3f (1.0f, 0.0f, 0.0f);
...
int
SiftGPU::IsFullSupported ()
{
  return GlobalUtil::_GoodOpenGL > 0 && GlobalUtil::_FullSupported;
}

void
SiftGPU::SaveSIFT (const char *szFileName)
{
  _pyramid->SaveSIFT (szFileName);
}

int
SiftGPU::GetFeatureNum ()
{
  return _pyramid->GetFeatureNum ();
}

void
SiftGPU::GetFeatureVector (SiftKeypoint *keys, float *descriptors)
{
...
void
SiftGPU::SetTightPyramid (int tight)
{
  GlobalUtil::_ForceTightPyramid = tight;
}

int
SiftGPU::AllocatePyramid (int width, int height)
{
...
void
SiftGPU::SetMaxDimension (int sz)
{
...
int
SiftGPU::GetImageCount ()
{
  return _list->size ();
}

void
SiftGPUEX::HSVtoRGB (float hsv[3], float rgb[3])
{
  int i;
  float q, t, p;
  float hh, f, v = hsv[2];
...
void
SiftGPUEX::GetImageDimension (int &w, int &h)
{
...
void
SiftGPUEX::GetInitWindowPotition (int &x, int &y)
{
...
/////////////////////////////////////////////////////
void *
ComboSiftGPU::operator new (size_t size)
{
  void *p = malloc (size);
