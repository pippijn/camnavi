...
PyramidCL::PyramidCL (SiftParam &sp)
  : SiftPyramid (sp)
{
  _allPyramid = NULL;
  ...
  if (GlobalUtil::_usePackedTex)
    _OpenCL = new ProgramBagCL ();
  else
    _OpenCL = new ProgramBagCLN ();
...
void
PyramidCL::InitializeContext ()
{
  GlobalUtil::InitGLParam (1);
}

void
PyramidCL::InitPyramid (int w, int h, int ds)
{
  int wp, hp, toobig = 0;
...
void
PyramidCL::ResizePyramid (int w, int h)
{
  //
  ...
  if (_allPyramid == NULL)
    {
      ...
      for (i = 0; i < texNum; ++i)
        _allPyramid[i].SetContext (context, queue);
    }
      ...
      for (j = 0; j < nlev; j++, gus++, dog++, grd++, rot++, key++)
        {
          ...
          if (j > 1 && j < nlev - 1)
            key->InitPackedTex (w, h, GlobalUtil::_usePackedTex);
        }
      ////////////////////////////////////////
...
void
PyramidCL::FitPyramid (int w, int h)
{
  _pyramid_octave_first = 0;
  //
      ...
      for (int j = param._level_min; j <= param._level_max; j++, tex++, dog++, grd++, rot++, key++)
        {
          ...
          if (j > param._level_min + 1 && j < param._level_max)
            key->SetPackedSize (w, h, GlobalUtil::_usePackedTex);
        }
      w >>= 1;
      h >>= 1;
    }
...
void
PyramidCL::SetLevelFeatureNum (int idx, int fcount)
{
...
int
PyramidCL::ResizeFeatureStorage ()
{
  int totalkb = 0;
  ...
  if (_hpLevelNum != num)
    {
      ...
      for (i = 0; i < _hpLevelNum; ++i)
        _histoPyramidTex[i].SetContext (context, queue);
    }
...
void
PyramidCL::GetFeatureDescriptors ()
{
  //descriptors...
...
void
PyramidCL::GenerateFeatureListTex ()
{
  vector<float> list;
  int idx = 0;
...
void
PyramidCL::ReshapeFeatureListCPU ()
{
  int i, szmax = 0, sz;
...
void
PyramidCL::GenerateFeatureDisplayVBO ()
{
...
void
PyramidCL::GenerateFeatureListCPU ()
{
...
void
PyramidCL::GenerateFeatureList (int i, int j, int reduction_count, vector<int> &hbuffer)
{
...
GLTexImage *
PyramidCL::GetLevelTexture (int octave, int level)
{
  return GetLevelTexture (octave, level, DATA_GAUSSIAN);
}

GLTexImage *
PyramidCL::ConvertTexCL2GL (CLTexImage *tex, int dataName)
{
  if (_bufferTEX == NULL)
  ...
  switch (dataName)
    {
    case DATA_GAUSSIAN:
      _OpenCL->UnpackImage (tex, &texCL);
      break;
    case DATA_DOG:
      _OpenCL->UnpackImageDOG (tex, &texCL);
      break;
    case DATA_GRAD:
      _OpenCL->UnpackImageGRD (tex, &texCL);
      break;
    case DATA_KEYPOINT:
      _OpenCL->UnpackImageKEY (tex,
                               tex - param._level_num * _pyramid_octave_num, &texCL);
      break;
    default:
      break;
    }
...
GLTexImage *
PyramidCL::GetLevelTexture (int octave, int level, int dataName)
{
...
void
PyramidCL::ConvertInputToCL (GLTexInput *input, CLTexImage *output)
{
...
void
PyramidCL::BuildPyramid (GLTexInput *input)
{
  USE_TIMING ();
          ...
          if (GlobalUtil::_usePackedTex)
            {
              ...
              if (i < 0)
                _OpenCL->SampleImageU (tex, _inputTex, -i - 1);
              else
                _OpenCL->SampleImageD (tex, _inputTex, i + 1);
            }
          else
            {
              if (i == 0)
                ConvertInputToCL (input, tex);
              else
                {
                  ConvertInputToCL (input, _inputTex);
                  if (i < 0)
                    _OpenCL->SampleImageU (tex, _inputTex, -i);
                  else
                    _OpenCL->SampleImageD (tex, _inputTex, i);
                }
            }
...
int
PyramidCL::FitHistogramPyramid (CLTexImage *tex)
{
  CLTexImage *htex;
  int hist_level_num = _hpLevelNum - _pyramid_octave_first / 2;
...
void
PyramidCL::GetFeatureOrientations ()
{
  /*
   *      CLTexImage * ftex = _featureTex;
...
void
PyramidCL::GetSimplifiedOrientation ()
{
...
CLTexImage *
PyramidCL::GetBaseLevel (int octave, int dataName)
{
