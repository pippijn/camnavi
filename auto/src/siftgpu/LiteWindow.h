...
public:
  ...
  void
  Create (int x = -1, int y = -1, const char *display = NULL)
  {
...
public:
  ...
  void
  Create (int x = 0, int y = 0, const char *display = NULL)
  {
    if (xDisplay)
      return;
    if (display)
      std::cout << "Using display [" << display << "]\n";
  ...
  {
    if (xContext)
      glXMakeCurrent (xDisplay, xWin, xContext);
  }
};
