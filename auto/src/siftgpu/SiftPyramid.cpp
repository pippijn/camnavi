...
void
SiftPyramid::RunSIFT (GLTexInput *input)
{
  CleanupBeforeSIFT ();
...
void
SiftPyramid::LimitFeatureCount (int have_keylist)
{
  if (GlobalUtil::_FeatureCountThreshold <= 0 || _existing_keypoints)
    return;
  ///////////////////////////////////////////////////////////////
  ...
  if (GlobalUtil::_TruncateMethod == 2)
    {
      ...
      for (; new_feature_num < _FeatureCountThreshold && i < level_num; ++i)
        new_feature_num += _levelFeatureNum[i];
      for (; i < level_num; ++i)
...
int
SiftPyramid::GetRequiredOctaveNum (int inputsz)
{
  //[2 ^ i,  2 ^ (i + 1)) -> i - 3...
...
void
SiftPyramid::CopyFeatureVector (float *keys, float *descriptors)
{
  if (keys)
    memcpy (keys, &_keypoint_buffer[0], 4 * _featureNum * sizeof (float));
  if (descriptors)
    memcpy (descriptors, &_descriptor_buffer[0], 128 * _featureNum * sizeof (float));
}

void
SiftPyramid::SetKeypointList (int num, const float *keys, int run_on_current, int skip_orientation)
{
  ...
  if (run_on_current)
    _existing_keypoints |= SIFT_SKIP_FILTERING;
  //orientation can be skipped if specified
  if (skip_orientation)
    _existing_keypoints |= SIFT_SKIP_ORIENTATION;
  //hacking parameter for using rectangle description mode
  if (skip_orientation == -1)
    _existing_keypoints |= SIFT_RECT_DESCRIPTION;
}

void
SiftPyramid::SaveSIFT (const char *szFileName)
{
  if (_featureNum <= 0)
    return;
              ...
              for (int k = 0; k < 128; k++, pd++)
                {
                  ...
                  if ((k + 1) % 20 == 0)
                    out << endl;                                        //suggested by Martin Schneider
                }
              out << endl;
            }
        }
...
#ifdef DEBUG_SIFTGPU
void
SiftPyramid::BeginDEBUG (const char *imagepath)
{
...
void
SiftPyramid::StopDEBUG ()
{
  _debug_path[0] = 0;
}

void
SiftPyramid::WriteTextureForDEBUG (GLTexImage *tex, const char *namet, ...)
{
  char name[_MAX_PATH];
