...
void
ShaderMan::InitShaderMan (SiftParam &param)
{
  if (s_bag)
    return;
...
void
ShaderMan::FilterImage (FilterProgram *filter, GLTexImage *dst, GLTexImage *src, GLTexImage *tmp)
{
  if (filter == NULL)
    return;
...
void
ShaderMan::FilterInitialImage (GLTexImage *tex, GLTexImage *buf)
{
  if (s_bag->f_gaussian_skip0)
    FilterImage (s_bag->f_gaussian_skip0, tex, tex, buf);
}

void
ShaderMan::FilterSampledImage (GLTexImage *tex, GLTexImage *buf)
{
  if (s_bag->f_gaussian_skip1)
    FilterImage (s_bag->f_gaussian_skip1, tex, tex, buf);
}

void
ShaderMan::TextureCopy (GLTexImage *dst, GLTexImage *src)
{
  dst->AttachToFBO (0);
...
void
ShaderMan::TextureDownSample (GLTexImage *dst, GLTexImage *src, int scale)
{
  //output parameter
...
void
ShaderMan::TextureUpSample (GLTexImage *dst, GLTexImage *src, int scale)
{
  //output parameter
  dst->AttachToFBO (0);
  //input parameter
...
{
  if (s_bag && s_bag->s_display_gaussian)
    s_bag->s_display_gaussian->UseProgram ();
}

void
ShaderMan::UseShaderDisplayDOG ()
{
  if (s_bag && s_bag->s_display_dog)
...
{
  if (s_bag && s_bag->s_gray)
    s_bag->s_gray->UseProgram ();
}

void
ShaderMan::UseShaderDisplayGrad ()
{
  if (s_bag && s_bag->s_display_grad)
    s_bag->s_display_grad->UseProgram ();
}

void
ShaderMan::UseShaderDisplayKeypoints ()
{
  if (s_bag && s_bag->s_display_keys)
    s_bag->s_display_keys->UseProgram ();
}

void
ShaderMan::UseShaderGradientPass (int texP)
{
...
void
ShaderMan::UseShaderKeypoint (int texU, int texD)
{
...
void
ShaderMan::UseShaderGenListInit (int w, int h, int tight)
{
  if (tight)
...
void
ShaderMan::UseShaderGenListStart (float fw, int tex0)
{
...
void
ShaderMan::UseShaderGenListStep (int tex, int tex0)
{
...
void
ShaderMan::UseShaderGenListEnd (int ktex)
{
...
{
  if (s_bag->s_debug)
    s_bag->s_debug->UseProgram ();
}

void
ShaderMan::UseShaderZeroPass ()
{
  if (s_bag->s_zero_pass)
    s_bag->s_zero_pass->UseProgram ();
}

void
ShaderMan::UseShaderGenVBO (float width, float fwidth, float size)
{
...
void
ShaderMan::UseShaderMarginCopy (int xmax, int ymax)
{
...
void
ShaderMan::UseShaderSimpleOrientation (int oTex, float sigma, float sigma_step)
{
...
void
ShaderMan::UseShaderOrientation (int gtex, int width, int height, float sigma, int auxtex, float step, int keypoint_list)
{
  s_bag->s_orientation->UseProgram ();
  ...
  if (keypoint_list)
    sigma = 0.0f;
  else if (GlobalUtil::_FixedOrientation)
    sigma = -sigma;
...
void
ShaderMan::UseShaderDescriptor (int gtex, int otex, int dwidth, int fwidth, int width, int height, float sigma)
{
...
void
ShaderMan::SelectInitialSmoothingFilter (int octave_min, SiftParam &param)
{
