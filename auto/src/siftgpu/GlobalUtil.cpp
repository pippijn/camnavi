...
int GlobalParam::_texMaxDim = 1000;                     //Maximum working size for SiftGPU, 3200 for packed
...
#ifdef _DEBUG
void
GlobalUtil::CheckErrorsGL (const char *location)
{
  GLuint errnum;
  const char *errstr;
...
void
GlobalUtil::SetDeviceParam (int argc, char **argv)
{
...
void
GlobalUtil::FitViewPort (int width, int height)
{
  GLint port[4];
...
void
ClockTimer::StartTimer (const char *event, int verb)
{
...
void
GlobalUtil::InitGLParam (int NotTargetGL)
{
...
int
GlobalUtil::CreateWindowEZ (LiteWindow *window)
{
  if (window == NULL)
    return 0;
  if (!window->IsValid ())
...
int
CreateLiteWindow (LiteWindow *window)
{
