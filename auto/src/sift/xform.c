 ...
 */
CvMat *
ransac_xform (struct feature *features, int n, int mtype,
 ...
 */
CvMat *
dlt_homog (CvPoint2D64f *pts, CvPoint2D64f *mpts, int n)
{
  CvMat *H, *A, *VT, *D, h, v9;
  double _h[9];
  int i;
 ...
 */
CvMat *
lsq_homog (CvPoint2D64f *pts, CvPoint2D64f *mpts, int n)
{
  CvMat *H, *A, *B, X;
  double x[9];
  int i;
 ...
 */
double
homog_xfer_err (CvPoint2D64f pt, CvPoint2D64f mpt, CvMat *H)
{
 ...
 */
CvPoint2D64f
persp_xform_pt (CvPoint2D64f pt, CvMat *T)
{
  CvMat XY, UV;
  double xy[3] = {
    pt.x, pt.y, 1.0
...
get_match (struct feature *feat, int mtype)
{
 ...
 */
static int
get_matched_features (struct feature *features, int n, int mtype,
 ...
 */
static int
calc_min_inliers (int n, int m, double p_badsupp, double p_badxform)
{
  double pi, sum;
  int i, j;
...
draw_ransac_sample (struct feature **features, int n,
                    int m)
{
 ...
 */
static void
extract_corresp_pts (struct feature **features, int n, int mtype,
 ...
 */
static int
find_consensus (struct feature **features, int n, int mtype,
 ...
 */
static inline void
release_mem (CvPoint2D64f *pts1, CvPoint2D64f *pts2,
