...
//global variable that holds the handler to setupapi.dll
...
#endif
{
  ...
  if(NULL == hSetupAPI) hSetupAPI = LoadLibrary(_T("SETUPAPI.DLL"));
  if (hSetupAPI == NULL)
    return FALSE;
...
bool CEnumerateSerial::unloadSetupapiLibrary() {
	if(NULL!=hSetupAPI)  {
		FreeLibrary(hSetupAPI);
		hSetupAPI = NULL;
	}
...
bool CEnumerateSerial::loadSetupapiLibrary() {
	if(NULL == hSetupAPI) hSetupAPI = LoadLibrary(_T("SETUPAPI.DLL"));
    if (hSetupAPI == NULL)
		return false;
	return true;
